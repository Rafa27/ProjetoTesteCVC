...
def substituir_moeda(valor)
    valor.delete("R$.").tr(',','.').to_f unless valor.equal? nil
end
...
# 				for count_row in 0...row_counter
# 					if nome_ciclo.nil? || nome_ciclo == 'N'
...
def recuperar_validar_data(data,formato = '')
  ## verificar formato da data a ser retornada
    case formato
    when '', 'dd/mm/yyyy'
      format_atual = '%d/%m/%Y'
    when 'aaaa-mm-dd'
  ...
  ## verificar data para retorno
    if data == ''
      data_retorno = ''
    elsif data.length == 8  #### assumindo: ddmmyyyy
      data_retorno = data[0,2] + '/' + data[2,2] + '/' + data[4,4]
    elsif data.length == 10 #### assumindo: dd/mm/yyyy
      data_retorno = data
    else
  ## verificar data parametrizada para retorno
      data = data.delete('(').delete(')')
      qtd_tamanho = data.length - 2
      aux_data = data[2,qtd_tamanho].to_i
      case data
      when /d-/, /D-/
        data_retorno = Date.today - aux_data
      when /d+/, /D+/
        data_retorno =  Date.today + aux_data
      else
  ...
  ### validar e retornar...
    if data_retorno.to_s != ''
      data_aux = Date.parse(data_retorno.to_s).strftime(format_atual)
      return data_aux.to_s
    else
      return data_retorno
    end
end
...
def texto_maiusculo(texto)
  return UnicodeUtils.upcase(texto)
end
